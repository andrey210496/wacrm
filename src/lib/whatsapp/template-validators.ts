...
export function validateTemplateName(name: string): void {
  ...
  if (!TEMPLATE_LIMITS.nameRegex.test(name)) {
    throw new Error(
      'O nome do modelo deve usar apenas letras minúsculas, dígitos e underscores (1-512 caracteres).',
    );
  }
}
...
function assertContiguous(indices: number[], where: string): void {
  for (let i = 0; i < indices.length; i++) {
    if (indices[i] !== i + 1) {
      throw new Error(
        `As variáveis de ${where} devem ser contíguas a partir de {{1}} — encontrado ${indices
          .map((n) => `{{${n}}}`)
          .join(', ')}.`,
      );
    }
...
export function validateBody(bodyText: string): number[] {
  ...
  if (bodyText.length > TEMPLATE_LIMITS.bodyMaxLength) {
    throw new Error(
      `O texto do corpo excede ${TEMPLATE_LIMITS.bodyMaxLength} caracteres (tem ${bodyText.length}).`,
    );
  }
...
export function validateFooter(footerText: string | undefined): void {
  ...
  if (footerText.length > TEMPLATE_LIMITS.footerMaxLength) {
    throw new Error(
      `O texto do rodapé excede ${TEMPLATE_LIMITS.footerMaxLength} caracteres (tem ${footerText.length}).`,
    );
  }
...
export function validateHeader(
  ...
  if (header_type === 'text') {
    ...
    if (header_content.length > TEMPLATE_LIMITS.headerTextMaxLength) {
      throw new Error(
        `O texto do cabeçalho excede ${TEMPLATE_LIMITS.headerTextMaxLength} caracteres (tem ${header_content.length}).`,
      );
    }
...
export function validateButtons(buttons: TemplateButton[] | undefined): void {
  ...
  if (buttons.length > TEMPLATE_LIMITS.maxButtonsTotal) {
    throw new Error(
      `Modelos podem ter no máximo ${TEMPLATE_LIMITS.maxButtonsTotal} botões (tem ${buttons.length}).`,
    );
  }
  ...
  if (counts.URL > TEMPLATE_LIMITS.maxUrlButtons) {
    throw new Error(
      `No máximo ${TEMPLATE_LIMITS.maxUrlButtons} botões URL permitidos (tem ${counts.URL}).`,
    );
  }
  if (counts.PHONE_NUMBER > TEMPLATE_LIMITS.maxPhoneButtons) {
    throw new Error(
      `No máximo ${TEMPLATE_LIMITS.maxPhoneButtons} botão PHONE_NUMBER permitido (tem ${counts.PHONE_NUMBER}).`,
    );
  }
  if (counts.COPY_CODE > TEMPLATE_LIMITS.maxCopyCodeButtons) {
    throw new Error(
      `No máximo ${TEMPLATE_LIMITS.maxCopyCodeButtons} botão COPY_CODE permitido (tem ${counts.COPY_CODE}).`,
    );
  }
  ...
  for (let i = 0; i < buttons.length; i++) {
    ...
    switch (b.type) {
      case 'URL': {
        if (!b.url?.trim()) {
          throw new Error(`O botão URL #${i + 1} está sem url.`);
        }
        try {
          new URL(b.url);
        } catch {
          throw new Error(`O botão URL #${i + 1} tem uma url inválida.`);
        }
        const urlVars = extractVariableIndices(b.url);
        if (urlVars.length > 1) {
          throw new Error(
            `O botão URL #${i + 1} pode ter no máximo uma variável (regra da Meta).`,
          );
        }
        if (urlVars.length === 1) {
          if (urlVars[0] !== 1) {
            throw new Error(
              `A variável do botão URL #${i + 1} deve ser {{1}} (regra da Meta).`,
            );
          }
          if (!b.example?.trim()) {
            throw new Error(
              `O botão URL #${i + 1} usa {{1}} — a Meta exige um valor de exemplo.`,
            );
          }
        }
        break;
...
export function validateSampleValues(
  ...
  if (body.length !== bodyVarCount) {
    throw new Error(
      `O corpo tem ${bodyVarCount} variável(is) — forneça exatamente ${bodyVarCount} valor(es) de exemplo (tem ${body.length}).`,
    );
  }
  if (header.length !== headerVarCount) {
    throw new Error(
      `O cabeçalho tem ${headerVarCount} variável(is) — forneça exatamente ${headerVarCount} valor(es) de exemplo (tem ${header.length}).`,
    );
  }
