...
describe('validateBody', () => {
  ...
  it('rejects non-contiguous variables', () => {
    expect(() => validateBody('Hi {{1}} {{3}}')).toThrow(/contíguas/);
  });
...
describe('validateButtons', () => {
  ...
  it('rejects > 2 URL buttons', () => {
    ...
    ).toThrow(/No máximo 2 botões URL/);
  });
  it('rejects > 1 PHONE_NUMBER', () => {
    ...
    ).toThrow(/No máximo 1 botão PHONE_NUMBER/);
  });
  it('rejects > 1 COPY_CODE', () => {
    ...
    ).toThrow(/No máximo 1 botão COPY_CODE/);
  });
  it('rejects QUICK_REPLY interleaved with CTA buttons', () => {
    ...
    ).toThrow(/não podem ser intercalados/);
  });
  ...
  it('rejects URL with {{1}} but no example', () => {
    ...
    ).toThrow(/a Meta exige um valor de exemplo/);
  });
