...
export default function NotificationsPage() {
  ...
  const load = useCallback(async () => {
    ...
    const { data, error: fetchErr } = await supabase
      ...
      .limit(100);
    if (fetchErr) {
      console.error("[notifications] load error:", fetchErr.message);
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="font-heading text-2xl font-bold tracking-tight text-foreground">Notificações</h1>
      ...
      {notifications.length === 0 ? (
        <div className="flex h-48 flex-col items-center justify-center rounded-xl border border-dashed border-border bg-muted/40">
          ...
          <p className="mt-1 text-xs text-muted-foreground">
            Você verá um aviso aqui quando alguém atribuir uma conversa a
            você.
          </p>
        </div>
      ) : (
